...
format: 
  html:
    ...
    page-layout: full
editor: source
bibliography: grateful-refs-tidying.bib
csl: apa.csl
number-sections: true
number-depth: 4
---

# (Install) and attache packages

```{r getAndAttachPackages}
#| message: false
#| warning: false
#| include: false
...
# Uncomment the code if you don't already have them installed
...
d01 <- read_csv(
  "responses_776cc413-2716-4170-987c-6f9b4658a726_RUNNING_2023-10-10101.csv",
  col_types =
    cols(
...
The `responseId` variable from *Pavlovia* provides unique IDs, but they're pretty unweildy for analysis, so they're being supplemented here with the rownames associated by *R* upon importing the data. This will be called `ID`. This also gives us the survey-completion order of the participants in the survey.
...
contributeLong$contributionType <- 
  fct_recode(
    contributeLong$contributionType,
...
contributeLong$contributionType <- fct_relevel(
  contributeLong$contributionType,
    "[I] contribute [by] improving documentation",
...
contributeLong <-
  ...
  mutate(., selected = fct_recode(selected, "yes" = "1", "no" = "0"))

contributeLong <-
  contributeLong %>%
  mutate(across(!c(gender, contributionType, selected), as.character)) %>%
  mutate(across(!c(gender, contributionType, selected),
              ~ fct_recode(.x, "yes" = "1", "no" = "0")))

contributeLong <- contributeLong |>
  var_labels(
    gender =
      "Which gender do you identify most with?",
    nonHeteroCommunity = 
...
```{r saveContributeLongToRDS}

saveRDS(contributeLong, "contributeLong.rds")

```

## deterrents

```{r pivotDeterrentsToLong}
...
```{r saveDeterrentsLongToRDS}

saveRDS(deterrentsLong, "deterrentsLong.rds")

```

## workshops

### with EDI variables, but not `gender`

```{r pivotWorkshopsToLong}

# This actually makes the EDI variables longer

workshopsLongNoGender <- tidiedData %>%
  select(
         nonHeteroCommunity,
         raceCulture,
         disability,
...
workshopsLongNoGender <- workshopsLongNoGender |>
  var_labels(
    ediVar =
...
```{r saveWorkshopLongToRDS}

saveRDS(workshopsLongNoGender, "workshopLong.rds")
...
allPackages <- 
  append(
    ...
    pkgs <- grateful::cite_packages(
      ...
      output = "table", 
      cite.tidyverse = F,
      bib.file = "grateful-refs-tidying.bib",
      Rmd.file = "grateful-report-tidying.Rmd",
      out.name = "grateful-citations-tidying"
      )
    knitr::kable(pkgs)
