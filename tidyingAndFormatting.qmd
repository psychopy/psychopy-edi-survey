...
# Prompt user to choose the input file
input_file <- tk_choose.files(
  default = file.path(here::here("dataRaw")),
...
d01 <- read_csv(
  input_file,
  col_types = cols(
    `deterrents.Lack of interest` = col_character(),
    `deterrents.Lack of time` = col_character(),
    `deterrents.Lack of knowledge on how to get started` = col_character(),
    `deterrents.Lack of coding skills` = col_character(),
    `deterrents.Not confident I would be welcomed by the community` = col_character(),
    `deterrents.Not aware contributing was possible` = col_character()
  ),
  na = ""
)

# ---- Tidy the data here ----
# tidiedData <- ... your processing code ...

# Create a unique output filename based on the input file name
input_filename <- basename(input_file)                            # e.g. "survey1_raw.csv"
output_base <- file_path_sans_ext(input_filename)                # e.g. "survey1_raw"
output_file <- file.path("dataFinal", paste0("tidyLabeled_", output_base, ".rds"))
...
write_tsv(
  tidiedData,
  file.path("dataFinal", paste0("anonymousSurveyData_", output_base, ".tsv"))
)
...
saveRDS(tidiedData, output_file)
...
saveRDS(
  longerDataSimple,
  file.path("dataFinal", paste0("longerDataSimple_", output_base, ".rds"))
)
...
saveRDS(
  longerDataContributions,
  file.path("dataFinal", paste0("longerDataContributions_", output_base, ".rds"))
)
...
saveRDS(
  longerDataDeterrents,
  file.path("dataFinal", paste0("longerDataDeterrents_", output_base, ".rds"))
)
